import unittest
import uuid
from osqltestutils import create_osql_cli_client, shutdown
from osqlcli.packages.special.main import special_command, execute, NO_QUERY
...
class SpecialCommandsTests(unittest.TestCase):
    ...
    table1 = 'osql_cli_table1_{0}'.format(session_guid)
    table2 = 'osql_cli_table2_{0}'.format(session_guid)
    view = 'osql_cli_view_{0}'.format(session_guid)
    database = 'osql_cli_db_{0}'.format(session_guid)
    schema = 'osql_cli_schema_{0}'.format(session_guid)
    index = 'osql_cli_index_{0}'.format(session_guid)
    function = 'osql_cli_func_{0}'.format(session_guid)
    login = 'osql_cli_login_{0}'.format(session_guid)
