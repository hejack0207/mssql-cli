...
from osqlcli.completion_refresher import CompletionRefresher
...
class osqlCli(object):
    ...
    def __init__(self, options):
        ...
        # Load config.
        c = self.config = get_config(options.osqlclirc_file)

        self.initialize_logging()
        self.logger = logging.getLogger(u'osqlcli.main')
    ...
    def initialize_logging(self):
        ...
        if log_file == 'default':
            log_file = config_location() + 'osqlcli.log'
        ...
        root_logger = logging.getLogger('osqlcli')
        root_logger.addHandler(handler)
        root_logger.setLevel(log_level)

        root_logger.info('Initializing osqlcli logging.')
        root_logger.debug('Log file %r.', log_file)

    def set_main_osqlcli_client(self, osqlcli_client):
        self.osqlcliclient_main = osqlcli_client

    def connect_to_database(self):

        try:
            owner_uri, error_messages = self.osqlcliclient_main.connect_to_database()
    ...
    def run(self):
        ...
        try:
            while True:
                ...
                # The reason we check here instead of inside the osqlcliclient is
                # because we want to raise the Exit exception which will be
                # caught by the try/except block that wraps the osqlcliclient execute
                # statement.
            ...
            self.osqlcliclient_main.shutdown()
            if not self.less_chatty:
                print('Goodbye!')
    ...
    def _build_cli(self, history):
        ...
        key_binding_manager = osqlcli_bindings(
    ...
    def _evaluate_command(self, text):
        ...
        for rows, columns, status, sql, is_error in \
                self.osqlcliclient_main.execute_query(text):
            ...
            if new_db_name:
                ...
                self.osqlcliclient_main.database = new_db_name
    ...
    def refresh_completions(self, history=None, persist_priorities='all'):
        # Clone osqlcliclient to create a new connection with a new owner Uri.
        osqlclclient_completion_refresher = self.osqlcliclient_main.clone()

        callback = functools.partial(self._on_completions_refreshed,
                                     persist_priorities=persist_priorities)

        self.completion_refresher.refresh(mssqcliclient=osqlclclient_completion_refresher,
                                          callbacks=callback,
                                          history=history,
    ...
    def _swap_completer_objects(self, new_completer, persist_priorities):
        ...
        with self._completer_lock:
            ...
            # When osql-cli is first launched we call refresh_completions before
